...
public final class TCB {
	...
	isFirstTCB = (currentTCB == null);//currentTCB是个静态变量
	...
	if (!isFirstTCB) {
	    ...
	    privilege.doPrivileged(new Runnable() {
		    public void run() { javaThread = new Thread(tcbTarget); }//这里是执行新建thread
		});
	    ...
	    this.javaThread.start();//新创建的java线程启动，这两句话的执行顺序
	     ...
	     */
	    javaThread = Thread.currentThread();//第一次使用当前的java线程
...
//	System.out.println(Thread.currentThread().toString());
     ...
     */
    private synchronized void waitForInterrupt() {//这里有个进程锁，确保只有当前线程去执行该方法
	while (!running) {//这里有个判断条件，当running是false时才执行
	    try { wait(); }//当前wait的是哪个线程
     ...
     * TCB.
     */
    private synchronized void interrupt() {//加一个锁确保只有一个线程在执行
	running = true;
	notify();//唤醒，这个地方不太明白
    }
     ...
     */
    private static TCB currentTCB = null;//静态变量
