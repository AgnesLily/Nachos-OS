...
public class PriorityScheduler extends Scheduler {
	...
	public KThread nextThread() {
	    ...
	    KThread thread = pickNextThread().thread;//得到下一个将要执行的线程
//	    System.out.println(thread.getName());
	    getThreadState(thread).acquire(this);//在等待队列中去除该线程
	    return thread;
	}
	 ...
	 *		return.
	 */
	protected ThreadState pickNextThread() {//选择具有最大优先级的线程执行
	    // implement me
...
//			System.out.println("下一个"+toNext.thread+" "+toNext.priority);
			if(toNext.getEffectivePriority() > next.getEffectivePriority()){
				next = toNext;
			}		
			//System.out.println(toNext.thread);
			//System.out.println("优先级是"+toNext.priority);			
		}
//		System.out.println("最终"+next.thread+" "+next.effectivePriority);
		//System.out.println("优先级是"+next.priority);
	    return next;
	}
    ...
    LinkedList<KThread> waitingQueue = new LinkedList<KThread>();//等待调度的线程
     ...
     */
    protected class ThreadState {//将线程与优先级联系起来
	/**
	...
	public int getEffectivePriority() {
		//System.out.println("找到有效优先级！！！！！！！！！！");
	    // implement me
//		if(effectivePriority != expiredEffectivePriority){
//			System.out.println("在这里返回了！！");
...
//		System.out.print(" "+effectivePriority);
	...
	public void waitForAccess(PriorityQueue waitQueue) {//传入的参数是当前系统中的readyQueue
	    // implement me
		//System.out.println(this.thread);
		//System.out.println("1111111");
		waitQueue.waitingQueue.add(this.thread);//在等待队列中加入该线程
...
//		getThreadState(this.thread).wait.remove(waitQueue);
		if(!waitQueue.transferPriority){//判断是否进行优先级翻转
	 ...
	 */
	public void acquire(PriorityQueue waitQueue) {//获得等待队列上的优先级
	    // implement me
		waitQueue.waitingQueue.remove(this.thread);
		waitQueue.lockHolder = this;
		waitQueue.lockHolder.effectivePriority = expiredEffectivePriority;
		waitQueue.lockHolder.waited = waitQueue;//将等待队列赋到资源所有者的threadState的waitqueue上，这些线程都在等待该lockHolder执行完获得资源
//		waitQueue.lockHolder.wait.
	...
	protected PriorityQueue waited = null;//等待该线程所占用资源的线程队列
	ThreadState lockHolder = null;
    }
}
